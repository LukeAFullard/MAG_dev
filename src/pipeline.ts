...
export interface VideoProcessingJob {
  ...
  clips?: ExtractedClip[];
  apparatus?: string;
  analysisMode?: 'rtmpose-s' | 'rtmpose-m' | 'rtmpose-l' | 'vitpose-s' | 'vitpose-b' | 'vitpose-l' | 'vitpose-h' | 'instanthmr';
}
...
export class PipelineManager {
  ...
  public async startJob(filename: string, file?: File, apparatus?: string, analysisMode: 'rtmpose-s' | 'rtmpose-m' | 'rtmpose-l' | 'vitpose-s' | 'vitpose-b' | 'vitpose-l' | 'vitpose-h' | 'instanthmr' = 'rtmpose-m'): Promise<string> {
  ...
  private async pass2_poseEstimation(jobId: string, signal: AbortSignal, file?: File): Promise<void> {
    ...
    for (let i = 0; i < job.clips.length; i++) {
      ...
      try {
        ...
        const poses = await poseExtractor.extract(file, clip, engine, job.analysisMode || 'rtmpose-m', (clipProgress) => {
          if (signal.aborted) return;
