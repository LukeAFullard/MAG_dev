...
const AppContent: React.FC = () => {
  ...
  const [selectedAttemptForAnnotation, setSelectedAttemptForAnnotation] =
    useState<any>(null);
  ...
  const handleVideoCaptured = async (file: File, apparatus: string, analysisMode: 'rtmpose-s' | 'rtmpose-m' | 'rtmpose-l' | 'vitpose-s' | 'vitpose-b' | 'vitpose-l' | 'vitpose-h' | 'instanthmr') => {
    try {
  ...
  const handleSavePoses = async (poses: any[]) => {
    ...
    try {
      const metrics = JSON.parse(
        selectedAttemptForAnnotation.metrics_json || "{}",
      );
      metrics.poses = poses;
      const updatedMetricsJson = JSON.stringify(metrics);

      await updateAttemptMetrics(
        selectedAttemptForAnnotation.id,
        updatedMetricsJson,
      );
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col">
      <header className="bg-slate-900 text-slate-50 shadow-md flex flex-col border-b border-slate-800">
        <div className="p-4 flex flex-col md:flex-row justify-between items-center">
          <div className="flex items-center gap-3">
            <ActivityIcon className="w-8 h-8 text-blue-500" />
            <h1 className="text-2xl font-bold tracking-tight">
              MAG_dev: Gymnastics Analysis
            </h1>
          </div>
          <div className="flex gap-4 mt-4 md:mt-0 text-sm font-medium">
            <div className="flex items-center gap-2">
              ...
              <span
                className={`px-2.5 py-1 rounded-full text-xs ${dbStatus === "Connected" ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : "bg-slate-800 text-slate-300"}`}
              >
                {dbStatus}
              </span>
            </div>
            <div className="flex items-center gap-2">
              ...
              <span
                className={`px-2.5 py-1 rounded-full text-xs ${inferenceStatus === "Ready" ? "bg-blue-500/10 text-blue-400 border border-blue-500/20" : "bg-slate-800 text-slate-300"}`}
              >
                {inferenceStatus}{" "}
                {isWebGPU !== null && `(${isWebGPU ? "WebGPU" : "WASM"})`}
              </span>
            </div>
          </div>
        </div>

        <nav className="flex px-4 overflow-x-auto border-t border-slate-800 bg-slate-900/50">
          <NavLink
            to="/overview"
            className={({ isActive }) =>
              `px-4 py-3 text-sm font-medium border-b-2 transition-colors whitespace-nowrap ${isActive ? "border-blue-500 text-blue-400" : "border-transparent text-slate-400 hover:text-slate-200"}`
            }
          >
            Overview
          </NavLink>
          ...
          >
            Analysis Tools{" "}
            {selectedAttemptForAnnotation && (
              <span className="ml-1 inline-flex w-2 h-2 rounded-full bg-blue-500"></span>
            )}
          </NavLink>
        </nav>
      </header>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 md:p-8">
                  <div className="mb-8">
                    <h2 className="text-2xl font-bold text-slate-900 mb-2">
                      Welcome to the Desktop-First Environment
                    </h2>
                    <p className="text-slate-600">
                      This is the initial scaffold for the local-first
                      gymnastics video analysis application. We use a robust
                      3-pass processing pipeline and secure local storage. Use
                      the navigation tabs above to manage your workflow.
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="bg-blue-50/50 rounded-xl p-5 border border-blue-100/50 hover:shadow-md transition-shadow">
                      ...
                      <p className="text-sm text-slate-600 ml-11">
                        Immediate motion detection and trimming.
                      </p>
                    </div>
                    <div className="bg-emerald-50/50 rounded-xl p-5 border border-emerald-100/50 hover:shadow-md transition-shadow">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <h3 className="font-bold text-slate-900">
                          Pass 2: Pose Analysis
                        </h3>
                      </div>
                      <p className="text-sm text-slate-600 ml-11">
                        Background skeletal tracking and COM extraction.
                      </p>
                    </div>
                    <div className="bg-purple-50/50 rounded-xl p-5 border border-purple-100/50 hover:shadow-md transition-shadow">
                      ...
                      <p className="text-sm text-slate-600 ml-11">
                        Constraint engine and metric calculations.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              element={
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 md:p-8">
                  ...
                  <form
                    ...
                    className="flex gap-3 mb-6 bg-slate-50 p-4 rounded-xl border border-slate-100"
                  >
                    <input
                      type="text"
                    ...
                    <button
                      type="submit"
                      className="bg-slate-900 text-white px-6 py-2.5 rounded-lg hover:bg-slate-800 font-medium shadow-sm transition-colors whitespace-nowrap"
                    >
                      Add Athlete
                    </button>
                  </form>
                  ...
                  <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
                    {athletes.length === 0 ? (
                      <div className="p-8 text-center text-slate-500 bg-slate-50">
                        No athletes found. Add one above to get started.
                      </div>
                    ) : (
                  ...
                  <div className="flex items-center justify-between mb-6">
                    <div>
                      ...
                      <p className="text-slate-500 mt-1">
                        Upload or record an attempt for automated extraction and
                        pose evaluation.
                      </p>
                    </div>
                    <CaptureGuidelines />
                    ...
                    <div className="space-y-4">
                      {jobs.map((job) => {
                        ...
                        return (
                          <div
                            key={job.id}
                            className={`border rounded-xl p-5 shadow-sm transition-all ${isComplete ? (isError ? "bg-red-50/50 border-red-100" : "bg-emerald-50/50 border-emerald-100") : "bg-white border-slate-200"}`}
                            ...
                            <div className="flex justify-between items-start mb-4">
                              ...
                              <div className="flex items-center gap-3">
                                {!isComplete && (
                                  <button
                                    onClick={() =>
                                      PipelineManager.getInstance().cancelJob(
                                        job.id,
                                      )
                                    }
                                ...
                                <span
                                  className={`text-xs font-medium px-2.5 py-1 rounded-full ${isComplete ? (isError ? "bg-red-100 text-red-700" : "bg-emerald-100 text-emerald-700") : "bg-blue-100 text-blue-700"}`}
                                >
                                  {job.status}
                                </span>
                              </div>
                            ...
                            {job.clips && job.clips.length > 0 && (
                              <div className="mt-5 border-t border-slate-200/60 pt-4">
                                <h5 className="text-xs font-semibold uppercase tracking-wider text-slate-500 mb-3">
                                  Extracted Clips ({job.clips.length})
                                </h5>
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                                  {job.clips.map((clip) => (
                                    <div
                                      key={clip.id}
                                      className="bg-white border border-slate-200 rounded-lg p-3 shadow-sm hover:border-blue-300 transition-colors"
                                      data-testid="extracted-clip"
                                    >
                                      ...
                                      {clip.landingMetrics && (
                                        <div className="mt-2 pt-2 border-t border-slate-100 grid grid-cols-2 gap-x-2 gap-y-1 text-xs text-slate-600">
                                          <div className="flex justify-between col-span-2">
                                            ...
                                            <span
                                              className={`font-medium ${clip.poses && clip.poses.length > 0 ? "text-emerald-600" : "text-red-600"}`}
                                            >
                                              {clip.poses &&
                                              clip.poses.length > 0
                                                ? "Yes"
                                          ...
                                          <div className="flex justify-between">
                                            <span>Impact</span>
                                            <span className="font-medium text-slate-900">
                                              {clip.landingMetrics.impactTime?.toFixed(
                                                2,
                                              )}
                                              s
                                            </span>
                                          ...
                                          <div className="flex justify-between">
                                            <span>Stab.</span>
                                            <span className="font-medium text-slate-900">
                                              {clip.landingMetrics.timeToStabilization?.toFixed(
                                                2,
                                              )}
                                              s
                                            </span>
                                          ...
                                          <div className="flex justify-between">
                                            <span>Steps</span>
                                            <span className="font-medium text-slate-900">
                                              {clip.landingMetrics.stepCount}
                                            </span>
                                          </div>
                                          <div className="flex justify-between">
                                            <span>Drift</span>
                                            <span className="font-medium text-slate-900">
                                              {clip.landingMetrics.lateralDrift?.toFixed(
                                                1,
                                              )}
                                              px
                                            </span>
                ...
                <div>
                  <SessionDashboard
                    onSelectAttempt={(attempt) => {
                      setSelectedAttemptForAnnotation(attempt);
                      // A hacky but effective way to navigate in the router without wrapping every item in a link
                      navigate("/analysis");
                    }}
                ...
                <div className="space-y-8">
                  {selectedAttemptForAnnotation ? (
                    <ManualAnnotation
                      ...
                      initialPoses={
                        JSON.parse(selectedAttemptForAnnotation.metrics_json)
                          ?.poses || []
                      }
                      onSavePoses={handleSavePoses}
                    />
                  ) : (
                    <div className="bg-white border p-12 rounded-xl text-center text-slate-500 shadow-sm">
                      <FileVideoIcon className="w-12 h-12 mx-auto text-slate-300 mb-4" />
                      <p className="text-lg font-medium text-slate-700">
                        No Attempt Selected
                      </p>
                      <p className="mt-2 text-sm">
                        Please select an attempt from the{" "}
                        <NavLink
                          to="/dashboard"
                        ...
                        to view or edit manual annotations.
                      </p>
                    </div>
                  )}
      ...
      <footer className="bg-slate-900 text-slate-400 p-6 text-center text-sm border-t border-slate-800">
        <p className="font-medium mb-1">
          MAG_dev - Gymnastics Analysis Assistant
        </p>
        <p className="text-xs opacity-75">
          Local-First Privacy-Compliant Analytics
        </p>
      </footer>
    </div>
  );
